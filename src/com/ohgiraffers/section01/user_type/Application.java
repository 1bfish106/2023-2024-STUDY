...
public class Application {
    public static void main(String[] args) {
        ...
        //사용자 정의 자료형 사용하기
        //변수 선언 및 객체생성
        //클래스명 변수명 = new 클래스명(); <- 지금까지 사용했던 이 구문은 객체를 생성하는 구문이다.
        //사용자 정의 자료형인 클래스를 이용하기 위해서는 new 연산자로 heap영역에 할당을 해야한다.
        //객체를 생성하게 되면 클래스에 정의한 필드와 메소드 대로 객체가 생성된다.

        Member member = new Member();

        //이렇게 객체를 생성하면 서로 다른 자료형을 member라는 하나의 이름으로 관리 할 수 있도록
        //공간을 생성한것이다. heap에 생성되므로 기본값으로 초기화한다.

        //필드에 접근하기 위해서는 레퍼런스변수명.필드명으로 접근한다.
        //'.'은 참조연산자라고 하는데 레퍼런스 변수가 참조하는 주소로 접근한다는 의미를 가진다.
